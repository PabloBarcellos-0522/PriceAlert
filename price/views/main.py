...
def index():
    ...
    if usuario_logado:
        ...
        notifications = Notification.query.filter_by(user_id=user_id).order_by(Notification.sent_at.desc()).limit(5).all()
        for n in notifications:
            best_offer = min(n.product.offers, key=lambda o: o.current_price) if n.product.offers else None
            monitoring = ProductMonitoring.query.filter_by(user_id=user_id, product_id=n.product_id).first()
            preco_alvo = float(monitoring.desired_price) if (monitoring and monitoring.desired_price) else 0.0
            alertas.append({
...
def dashboard():
    ...
    user_id = session['user_id']
    total_monitorados = ProductMonitoring.query.filter_by(user_id=user_id, is_active=True).count()
    total_alertas = Notification.query.filter_by(user_id=user_id).count()

    economia_total = 0.0
    monitorings = ProductMonitoring.query.filter_by(user_id=user_id, is_active=True).all()
    total_lojas = 0
...
def search():
    ...
    if termo_busca:
        ...
        if not produtos:
            ...
            try:
                produtos = current_app.product_service.search(termo_busca, fetch_offers=False)
                fonte = 'api'
            except Exception as e:
                db.session.rollback()
                current_app.logger.error(f"Erro ao buscar produtos via SerpAPI: {e}")
...
def adicionar_monitoramento():
    ...
    try:
        ...
        flash(f"Produto '{product.title}' adicionado aos monitoramentos!", "success")
    except Exception as e:
