...
from price.tasks.scanner import run_weekly_price_scanner
...
@pytest.fixture
def inactive_monitoring(db, test_user):
    """Cria e retorna um monitoramento inativo com um produto específico."""
    product = Product(
        google_product_id="prod_inactive",
        title="Produto Inativo",
        str_current_price="R$ 100,00",
        product_token="token_inactive"
    )
...
# =============================================================================
# TESTES DE TASKS/SCANNER (LOGICA ISOLADA DE ESCANEAMENTO)
# =============================================================================

def test_run_weekly_price_scanner_no_active_products(app, db):
    """Testa se a task run_weekly_price_scanner retorna cedo sem escanear se não houver produtos ativos."""
    db.session.query(ProductMonitoring).update(
        {ProductMonitoring.is_active: False})
    db.session.commit()
...
def test_run_weekly_price_scanner_success_flow(app, db, test_user, test_product, active_monitoring):
    """Testa se a task run_weekly_price_scanner atualiza o campo updated_at e executa com sucesso."""
    test_product.updated_at = None
    db.session.commit()

    def mock_get_details_side_effect(*args, **kwargs):
        test_product.updated_at = datetime.now(UTC)
    mock_get_details = MagicMock(side_effect=mock_get_details_side_effect)
    mock_notification = Notification(
        user=test_user,
        product=test_product,
        title="Alerta de Preço!",
...
def test_run_weekly_price_scanner_product_without_token(app, db, test_user, active_monitoring):
    """Testa se a task run_weekly_price_scanner avisa e continua se um produto ativo não tiver token."""
    active_monitoring.product.product_token = None
    db.session.commit()
...
def test_run_weekly_price_scanner_partial_failure(app, db, test_user):
    """Testa se a falha individual de um produto não impede a task de processar outros produtos."""
    p1 = Product(google_product_id="prod_t1",
                 title="Prod T1", product_token="token_t1")
    p2 = Product(google_product_id="prod_t2",
                 title="Prod T2", product_token="token_t2")
    db.session.add_all([p1, p2])
    db.session.commit()
...
def test_run_weekly_price_scanner_critical_failure(app, db, test_user, active_monitoring):
    """Testa o tratamento de falha crítica na task disparando rollback do banco de dados."""
    mock_get_details = MagicMock()

    with patch.object(app.product_service, 'get_product_details', mock_get_details), \
            patch.object(db.session, 'commit', side_effect=Exception("Database error")) as mock_commit, \
            patch.object(db.session, 'rollback') as mock_rollback:
